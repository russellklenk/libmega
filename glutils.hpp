...
/// enumerations and performing pixel transfer operations. The implementation
/// targets the OpenGL 3.3 core profile.
...
//  Functions  //
///////////////*/
/// @summary Given an OpenGL data type value (typically the type of a uniform)
/// determines the size of the data in bytes.
/// @param data_type The OpenGL data type, for example, GL_FLOAT_VEC4.
/// @return The size of the data, in bytes, or zero if the type is unknown.
size_t data_size(GLenum data_type);
...
/// @summary Allocates storage for a buffer object. This is really just a
/// wrapper around glBufferData(), and is provided for interface consistency.
/// @param target The buffer bind target, for example, GL_ARRAY_BUFFER.
/// @param usage The buffer usage hint to the driver, which helps the driver
/// determine the best type of memory, for example, GL_STREAM_DRAW.
/// @param capacity The buffer capacity, specified in bytes.
void buffer_storage(GLenum target, GLenum usage, size_t capacity);

/// @summary Allocates new storage for a buffer object. The existing storage is
/// orphaned and will be freed by the driver when no draw commands reference
/// it. This just calls buffer_storage(), but is named differently to make the
/// intent clear. Note that buffers can also be orphaned on-map by specifying
/// the GL_MAP_INVALIDATE_BUFFER_BIT flag.
/// @param target The buffer bind target, for example, GL_ARRAY_BUFFER.
/// @param usage The buffer usage hint to the driver, which helps the driver
/// determine the best type of memory, for example, GL_STREAM_DRAW.
/// @param capacity The buffer capacity, specified in bytes. This must be the
/// same capacity that was specified during the initial buffer_storage() call.
void orphan_buffer(GLenum target, GLenum usage, size_t capacity);
