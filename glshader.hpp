...
bool assign_vertex_attributes(
    GLuint       program,
    char const **attrib_names,
    GLuint      *attrib_locations,
    size_t       attrib_count);
...
/// @return true if the bindings were assigned without error.
bool assign_fragment_outputs(
    GLuint       program,
    char const **output_names,
    GLuint      *output_locations,
    size_t       output_count);
...
/// @summary Binds a texture object to a texture sampler for the currently 
    ...
    size_t          count)
{
    ...
    return kv_find(name_u32, name_list, value_list, count);
}
...
{
    return kv_find(name, shader->AttributeNames, shader->Attributes, shader->AttributeCount);
}
...
{
    return kv_find(name, shader->SamplerNames, shader->Samplers, shader->SamplerCount);
}
...
{
    return kv_find(name, shader->UniformNames, shader->Uniforms, shader->UniformCount);
}
